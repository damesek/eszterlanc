...
//					System.out.println(word + " " + tag + " " + lexiconScores[i]);
//				}
				if (scale)
		...
		for (int start = 0; start < length; start++) {
			for (int end = start + 1; end <= length; end++) {
				for (int state = 0; state < numStates; state++) {
					...
					if (allowedSubStates[start][end][state] == null)
						continue;
					for (int substate = 0; substate < getGrammar().numSubStates[state]; substate++) {
						if (allowedSubStates[start][end][state][substate]) {
							onePossible = true;
							break;
						}
	...
	public double getLogLikelihood(Tree<String> tree) {
		...
		StateSetTreeList resultStateSetTrees = new StateSetTreeList(resultList,
				getGrammar().numSubStates, false, tagNumberer);
		if (llParser == null)
			llParser = new ArrayParser(getGrammar(), getLexicon());
	...
	public Tree<String> extractBestMaxRuleParse1(int start, int end, int state,
		...
		if (cState == -1) {
			...
			int intermediateNode = getGrammar().getUnaryIntermediate(
	...
	public Tree<String> extractBestViterbiParse(int gState, int gp, int start,
		...
		UnaryRule[] unaries = getGrammar().getClosedViterbiUnaryRulesByParent(
				gState);
		for (int r = 0; r < unaries.length; r++) {
			UnaryRule ur = unaries[r];
			int cState = ur.childState;
	...
	public CoarseToFineMaxRuleParser newInstance() {
		CoarseToFineMaxRuleParser newParser = new CoarseToFineMaxRuleParser(
				getGrammar(), getLexicon(), unaryPenalty, endLevel,
				viterbiParse, outputSub, outputScore, accurate,
		...
		if (sumScores) {
			for (int pState = 0; pState < numStates; pState++) {
				if (allowedStates[start][end + 1][pState] == false)
					continue;
				for (int cp = 0; cp < getGrammar().numSubStates[pState]; cp++) {
