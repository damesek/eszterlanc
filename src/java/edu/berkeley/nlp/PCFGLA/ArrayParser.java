...
public class ArrayParser implements Parser, Serializable {
	...
	private static final long serialVersionUID = 8856890440922709107L;
	...
	void initializeChart(List<String> sentence, boolean noSmoothing) {
		...
		for (String word : sentence) {
			...
			for (short tag = 0; tag < numStates; tag++) {
				if (getGrammar().isGrammarTag[tag])
					continue;
				double prob = getLexicon().score(word, tag, start, noSmoothing,
						false)[0];
	...
	void doInsideScores() {
		getGrammar().logarithmMode();
		getLexicon().logarithmMode();
		...
		for (int diff = 1; diff <= length; diff++) {
			...
			for (int start = 0; start < (length - diff + 1); start++) {
				...
				for (int pparentState = 0; pparentState < numStates; pparentState++) {
					BinaryRule[] parentRules = getGrammar()
	...
	private void doOutsideScores() {
		...
		for (int diff = length; diff >= 1; diff--) {
			for (int start = 0; start + diff <= length; start++) {
				...
				// do binaries
				for (int s = 0; s < numStates; s++) {
					BinaryRule[] rules = getGrammar().splitRulesWithP(s);
					for (int r = 0; r < rules.length; r++) {
	...
	void doInsideScores(Tree<StateSet> tree, boolean noSmoothing,
		...
		if (getGrammar().isLogarithmMode() || getLexicon().isLogarithmMode())
			throw new Error(
		...
		if (tree.isPreTerminal()) {
			...
			double[] lexiconScores = getLexicon().score(wordStateSet, pState,
					noSmoothing, false);
			...
			case 1:
				...
				double[][] uscores = getGrammar().getUnaryScore(pState, cState);
				...
				if (debugOutput && !foundOne) {
					...
					System.out.println(getGrammar().getUnaryRule(pState, cState));
			...
			case 2:
				...
				double[][][] bscores = getGrammar().getBinaryScore(pState, lState,
						rState);
				...
				if (debugOutput && !foundOne2) {
					System.out.println("iscore reached zero!");
					System.out.println(getGrammar().getBinaryRule(pState, lState,
							rState));
	...
	public Tree<String> extractBestParse(int goal, int start, int end,
		...
		if (end - start == 1) {
			...
			if (!getGrammar().isGrammarTag[goal]) {
			...
			// the best way to get from the goal tag to a preterminal tag
				...
				for (int r = 0; r < unaries.length; r++) {
					...
					double ruleScore = iScore[start][end][ur.childState]
							+ getGrammar().getUnaryScore(ur)[0][0];
					if ((ruleScore > veryBestScore) && (goal != ur.childState)
							&& (!getGrammar().isGrammarTag[ur.getChildState()])) {
		...
		for (int split = start + 1; split < end; split++) {
			BinaryRule[] parentRules = getGrammar().splitRulesWithP(goal);
			for (int i = 0; i < parentRules.length; i++) {
		...
		// check unaries
		UnaryRule[] unaries = getGrammar().getClosedViterbiUnaryRulesByParent(goal);
		for (int r = 0; r < unaries.length; r++) {
			UnaryRule ur = unaries[r];
	...
	protected void restoreUnaries(Tree<String> t) {
		...
		for (Iterator nodeI = t.subTreeList().iterator(); nodeI.hasNext();) {
			...
			List<short[]> path = getGrammar().getBestViterbiPath(pLabel, (short) 0,
					cLabel, (short) 0);
	...
	public void doViterbiInsideScores(Tree<StateSet> tree) {
		...
		StateSet parent = tree.getLabel();
		short pState = parent.getState();
		int nParentStates = getGrammar().numSubStates[pState];// parent.numSubStates();
		double[] iScores = new double[nParentStates];
		if (tree.isPreTerminal()) {
			// Plays a role similar to initializeChart()
			String word = tree.getChildren().get(0).getLabel().getWord();
			int pos = tree.getChildren().get(0).getLabel().from;
			iScores = getLexicon().score(word, pState, pos, false, false);
			// parent.scaleIScores(0);
		} else {
			...
			case 2:
				StateSet leftChild = children.get(0).getLabel();
				StateSet rightChild = children.get(1).getLabel();
				int nLeftChildStates = getGrammar().numSubStates[leftChild
						.getState()];// leftChild.numSubStates();
				int nRightChildStates = getGrammar().numSubStates[rightChild
						.getState()];// rightChild.numSubStates();
				short lState = leftChild.getState();
				short rState = rightChild.getState();
				double[][][] bscores = getGrammar().getBinaryScore(pState, lState,
						rState);
				for (BinaryRule br : getGrammar().splitRulesWithP(pState)) {
	...
	public void incrementExpectedGoldCounts(Linearizer linearizer,
		...
		if (tree.isPreTerminal()) {
			...
			final int nSubStates = getGrammar().numSubStates[tag];
		...
		int nParentSubStates = getGrammar().numSubStates[parentState];
		...
		case 1:
			...
			int curInd = 0;
			int nChildSubStates = getGrammar().numSubStates[childState];
			UnaryRule urule = getGrammar().getUnaryRule(parentState, childState);
		...
		case 2:
			...
			int nLeftChildSubStates = getGrammar().numSubStates[lChildState];
			int nRightChildSubStates = getGrammar().numSubStates[rChildState];
			...
			BinaryRule brule = getGrammar().getBinaryRule(parentState, lChildState,
					rChildState);
	...
	public void countPosteriors(double[][] cumulativePosteriors,
		...
		final int nSubStates = getGrammar().numSubStates[state];
