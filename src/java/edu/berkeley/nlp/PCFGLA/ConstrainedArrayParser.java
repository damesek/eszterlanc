...
public class ConstrainedArrayParser extends ArrayParser implements Callable {
	...
	public ConstrainedArrayParser newInstance() {
		ConstrainedArrayParser newParser = new ConstrainedArrayParser(getGrammar(),
				getLexicon(), numSubStatesArray);
		return newParser;
	}
	...
	void initializeChart(List<String> sentence) {
		...
		for (String word : sentence) {
			...
			for (short tag = 0; tag < getGrammar().numSubStates.length; tag++) {
				...
				double[] lexiconScores = getLexicon().score(word, tag, start, false,
						false);
			...
			for (int start = 0; start < (length - diff + 1); start++) {
				...
				for (int state : possibleSt) {
					if (scores[start][end][state] != null) {
						for (int s = 0; s < getGrammar().numSubStates[state]; s++) {
							Numberer n = getGrammar().tagNumberer;
							System.out.print("("
			...
			// transform it into one
				...
				for (int r = 0; r < unaries.length; r++) {
					...
					for (short cp = 0; cp < scores.length; cp++) {
						...
						if ((ruleScore >= veryBestScore)
								&& (gState != cState || gp != cp)
								&& !getGrammar().isGrammarTag(cState)) {
							veryBestScore = ruleScore;
							newIndex = cState;
							newSubstate = cp;
						}
